...
class SLMApp:
    def __init__(self, master):
        self.master = master
        master.title("GeoStatistical Tool — SLM")
        ...
        self.train_button = tk.Button(frame, text="Train SLM", command=self.train_model, width=30)
        self.train_button.grid(row=4, column=0, pady=15)

        self.predict_button = tk.Button(frame, text="Run Saved SLM", command=self.predict_with_saved_model, width=30)
    ...
    def train_model(self):
        ...
        try:
            ...
            w.transform = 'r'
            slm = ML_Lag(y, X_scaled, w=w, name_y=dep_var, name_x=indep_vars)  # Spatial Lag (no Durbin)
            gdf_clean['Predicted'] = slm.predy.flatten()
            gdf_clean['Residuals'] = slm.u.flatten()
    ...
    def predict_with_saved_model(self):
        model_path = filedialog.askopenfilename(filetypes=[("Joblib files", "*.joblib")], title="Select Saved SLM Model")
        if not model_path:
            return
        ...
        try:
            ...
            gdf['Prediction'] = slm.predy.flatten()

            out_path = os.path.splitext(shp_path)[0] + "_slm_predicted.shp"
            gdf.to_file(out_path)
            messagebox.showinfo("Success", f"SLM prediction completed.\nSaved to: {out_path}")
        except Exception as e:
    ...
    def generate_report(self, data, gdf_clean, report_path):
        ...
        c.drawString(50, height - 50, "SLM Regression Report")
