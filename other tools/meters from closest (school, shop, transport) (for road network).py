...
def select_files_and_run(root):
    ...
    try:
        ...
        for idx, row in road_gdf.iterrows():
            ...
            for typ in types:
                filtered = poi_gdf[poi_gdf["fclass"].str.lower() == typ]
                if filtered.empty:
                    continue

                filtered = filtered.copy()
                filtered["dist"] = filtered.geometry.distance(mid_pt)
                nearest_pois = filtered.nsmallest(3, "dist")

                for i, (_, poi_row) in enumerate(nearest_pois.iterrows(), 1):
                    field = f"{typ.upper()}{i}"
                    ...
                    end_node = add_virtual_node_on_line(G, line_for_poi, poi_point, f"{road_name}_{typ}_{i}")

                    try:
                        if start_node is None or end_node is None or not nx.has_path(G, start_node, end_node):
                            raise Exception("Routing path failed")

                        length = nx.shortest_path_length(G, source=start_node, target=end_node, weight='length')
                        road_gdf.at[idx, field] = round(length, 2)
                        print(f"✅ {road_name} → {field} = {length:.2f} m")
                    except Exception as e:
                        fallback = geodesic((mid_pt.y, mid_pt.x), (poi_point.y, poi_point.x)).meters
                        road_gdf.at[idx, field] = round(fallback, 2)
                        print(f"⚠️ {road_name}: fallback {field} = {fallback:.2f} m ({e})")

        output_path = os.path.join(output_dir, "road_lines_with_poi_distances.shp")
