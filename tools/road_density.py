...
def is_batch_config_complete(config):
    ...
    This tool has ONE secondary source (Road Network Source, no dynamic
    checklist beneath it) PLUS a Buffer Radius setting -- both stay
    visible in batch mode (only Land Parcel Source / Output Destination
    are hidden). A batch config is complete once BOTH a Road Network
    source is selected AND the buffer radius is a valid positive
    number -- mirrors _is_valid_buffer()'s own acceptance rule inside
    open_main_window() (float, > 0) exactly, so this never diverges
    from what on_run() would itself accept.

    Args:
        config (dict): this tool's own last-Saved batch config, in the
            shape _gather_batch_config() below produces:
            {"road_source_type": "local", "road_local_path": "...",
             "buffer_radius": "1000"} or
            {"road_source_type": "db", "road_db_table": "...",
             "buffer_radius": "1000"}.

    Returns:
        bool: True iff a Road Network source is present for whichever
            mode the config specifies AND buffer_radius is a valid
            positive number.
    """
    if not config:
        return False
    if config.get("road_source_type") == "local":
        ...
        return False
    try:
        buffer_ok = float(config.get("buffer_radius", "")) > 0
    except (TypeError, ValueError):
        buffer_ok = False
    return has_road and buffer_ok
...
def open_main_window(root, db_verified=True, batch_mode=False,
    ...
    Args:
        ...
        batch_mode: bool, default False -- NEW. When True, the Land
            ...
            Valuation orchestrator instead); Road Network Source and
            Buffer Radius are the only sections still shown, since
            those are this tool's only other settings; "Run
            Processing" is replaced by "Save" / "Cancel". See
        ...
        initial_config: dict | None, default None -- NEW. Pre-fills
            Road Network Source and Buffer Radius (this tool's only
            batch-visible fields) when re-opening a previously-saved
            batch entry -- see is_batch_config_complete()'s own
            docstring for the config dict's shape.
        on_save: callable(config: dict) -> None, default None -- NEW.
            Called (before the window closes) when batch_mode=True and
            the user clicks Save, with the currently selected Road
            Network source and buffer radius -- see
            _gather_batch_config() below. Works even if incomplete --
            batch mode never blocks Save on completeness.
        on_cancel: callable() -> None, default None -- NEW. Called
            (before the window closes) when batch_mode=True and the
    ...
    win = tk.Toplevel(root)
    apply_icon(win, "roaddensity.ico")
    win.title("Road Density Tool" + (" — Batch Mode" if batch_mode else ""))
    win.resizable(False, False)
    win.update_idletasks()
    win.deiconify()
    win.lift()
    ...
    # NEW -- batch mode: pre-fill Road Network Source and Buffer Radius
    # from a previously-Saved config (see is_batch_config_complete()'s
    # own docstring for the dict shape). No-op when initial_config is
    # None (first Edit this session) or when batch_mode is False.
    if batch_mode and initial_config:
        ...
        if initial_config.get("buffer_radius"):
            buffer_var.set(initial_config["buffer_radius"])

    # NEW -- placeholder assigned BEFORE Section 2 (Road Network) is
    # built below. Section 2's and Section 3's own handlers
    # (browse_road_file(), _on_road_db_selected(), _toggle_road(), the
    # buffer_var trace) call _update_run_button_state() unconditionally
    # -- the SAME code whether batch_mode is True or False, since Road
    # Network Source and Buffer Radius are the two sections this tool
    # always builds. In batch mode there is no Run button to update, so
    # this stays a no-op; the REAL implementation (further below) is
    # only defined -- shadowing this one -- inside the
    # `if not batch_mode:` branch. Safe: Python resolves this name at
    # CALL time (when the user actually interacts with a widget, after
    # the whole window is built), not at each function's own definition
    # time.
    ...
    # NEW -- applied regardless of batch_mode: Road Network Source and
    # Buffer Radius are the two sections this tool always builds, so
    # Road Network's own initial label sync, its own Database radio
    # gating, and the buffer field's own live-update trace all happen
    # here, shared by both branches below, rather than duplicated in
    # each.
    _toggle_road()
    if not db_verified:
    ...
    if not batch_mode:
        ...
        # NEW -- batch mode: Road Network Source and Buffer Radius
        # (built above, unconditionally) are this tool's only
        # batch-visible sections. "Save" / "Cancel" replace "Run
        # Processing" -- Save works even if incomplete, per the
        # batch-mode contract (never blocks Save on completeness); the
        # orchestrator's own "Incomplete" overlay (via
        # is_batch_config_complete() above) is what actually reflects
        # readiness, not this window.
        ttk.Separator(win, orient="horizontal").pack(
            fill="x", padx=10, pady=(12, 4))

        def _gather_batch_config():
            config = {"buffer_radius": buffer_var.get()}
            if road_source_type.get() == "local":
        ...
        try:
            while True:
                ...
                if kind == "update":
                    ...
                    progress.close()
                    show_tool_dialog(root, "Success", rest[0], body_icon="success")
                    return
                elif kind == "error":
                    progress.close()
                    show_tool_dialog(root, "Error", rest[0], body_icon="error")
                    return
                elif kind == "cancelled":
                    progress.close()
                    show_tool_dialog(root, "Cancelled", "The run was cancelled. No data was changed.", body_icon="cancelled")
                    return
        except queue.Empty:
            pass
