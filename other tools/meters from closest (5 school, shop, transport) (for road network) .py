...
def select_files_and_run(root):
    ...
    try:
        ...
        for idx, row in road_gdf.iterrows():
            ...
            for typ in types:
                filtered = poi_gdf[poi_gdf["type"].str.lower() == typ]
                if filtered.empty:
                    continue

                filtered = filtered.copy()
                filtered["geo_dist"] = filtered.geometry.distance(mid_pt)
                nearest_candidates = filtered.nsmallest(10, "geo_dist")

                poi_distances = []
                for _, poi_row in nearest_candidates.iterrows():
                    poi_point = poi_row.geometry
                    closest_seg_idx = road_gdf.distance(poi_point).idxmin()
                    line_for_poi = road_gdf.loc[closest_seg_idx].geometry
                    end_node = add_virtual_node_on_line(G, line_for_poi, poi_point, f"{road_name}_{typ}")

                    try:
                        if start_node and end_node and nx.has_path(G, start_node, end_node):
                            length = nx.shortest_path_length(G, source=start_node, target=end_node, weight='length')
                        else:
                            raise Exception("Path not found")
                    except:
                        length = geodesic((mid_pt.y, mid_pt.x), (poi_point.y, poi_point.x)).meters
                        print(f"⚠️ {road_name} fallback for {typ.upper()} POI at ({poi_point.x:.5f}, {poi_point.y:.5f}): {length:.2f} m")

                    poi_distances.append(length)

                    if len(poi_distances) >= 5:
                        break

                poi_distances = sorted(poi_distances)[:5]
                for i in range(5):
                    field = f"{typ.upper()}{i+1}"
                    value = round(poi_distances[i], 2) if i < len(poi_distances) else None
                    road_gdf.at[idx, field] = value
                    if value is not None:
                        print(f"✅ {road_name} → {field} = {value:.2f} m")

        output_path = os.path.join(output_dir, "road_lines_with_5nearest_pois.shp")
...
def launch_gui():
    root = tk.Tk()
    root.title("Meters From Road Midpoint")
