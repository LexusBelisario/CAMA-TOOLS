...
def select_files_and_run(root):
    ...
    try:
        ...
        tree = cKDTree(np.c_[poi_proj["x"], poi_proj["y"]])
        road_gdf["200m_poi"] = 0
        ...
        for idx, row in road_gdf.iterrows():
            ...
            indices = tree.query_ball_point([mid_x, mid_y], r=200)
            if not indices:
                print("   ↪ POIs within 200m: 0")
                continue
            ...
            for _, poi_row in nearby.iterrows():
                ...
                closest_idx = road_proj.distance(poi_geom).idxmin()
                line_for_poi = road_gdf.loc[closest_idx].geometry
                end_node = add_virtual_node_on_line(G, line_for_poi, poi_geom, f"{road_name}_poi")
            ...
            road_gdf.at[idx, "200m_poi"] = count
            print(f"   ↪ POIs within 200m: {count}")

        output_path = os.path.join(output_dir, "road_lines_with_200m_all_poi.shp")
...
def launch_gui():
    root = tk.Tk()
    root.title("Count All POIs within 200m of Road Midpoints")
