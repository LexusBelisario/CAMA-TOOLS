...
def select_files_and_run(root):
    ...
    try:
        ...
        # Filter "other" POIs (exclude school, shop, transport)
        excluded = ["school", "shop", "transport"]
        poi_gdf = poi_gdf[~poi_gdf["fclass"].str.lower().isin(excluded)]

        # Prepare projected layers
        ...
        # Build KDTree for all "other" POIs
        poi_tree = cKDTree(np.c_[poi_proj["x"], poi_proj["y"]])
        road_gdf["5km_other"] = 0
        ...
        for idx, row in road_gdf.iterrows():
            ...
            indices = poi_tree.query_ball_point([mid_x, mid_y], r=5000)
            if not indices:
                print(f"   ↪ OTHER POIs within 5km: 0")
                continue

            count = 0
            nearby = poi_proj.iloc[indices]

            for _, poi_row in nearby.iterrows():
                poi_geom = poi_row.geometry
                poi_proj_geom = poi_geom
                closest_idx = road_proj.distance(poi_proj_geom).idxmin()
                line_for_poi = road_gdf.loc[closest_idx].geometry
                end_node = add_virtual_node_on_line(G, line_for_poi, poi_geom, f"{road_name}_other_poi")
            ...
            road_gdf.at[idx, "5km_other"] = count
            print(f"   ↪ OTHER POIs within 5km: {count}")

        output_path = os.path.join(output_dir, "road_lines_with_5km_other_poi_counts.shp")
...
def launch_gui():
    root = tk.Tk()
    root.title("Count OTHER POIs within 5km of Road Midpoints")
