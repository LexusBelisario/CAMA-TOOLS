...
def is_batch_config_complete(config):
    ...
    This tool has TWO secondary sources -- Road Network Source and DTM
    ...
    BOTH a Road Network source AND a DTM source are selected -- mirrors
    on_run()'s own validation for each exactly (a non-empty path/table
    for whichever mode each one specifies).
    ...
    Args:
        config (dict): this tool's own last-Saved batch config, in the
            ...
            {"road_source_type": "local"|"db",
             ...
             "dtm_source_type": "local"|"db",
             "dtm_local_path"|"dtm_db_table": "..."}.

    Returns:
        bool: True iff both a Road Network source and a DTM source are
    ...
    if config.get("road_source_type") == "local":
        ...
        return False
    if config.get("dtm_source_type") == "local":
        has_dtm = bool(config.get("dtm_local_path"))
    elif config.get("dtm_source_type") == "db":
        has_dtm = bool(config.get("dtm_db_table"))
    else:
        return False
    return has_road and has_dtm
...
def open_main_window(root, db_verified=True, batch_mode=False,
    ...
    Args:
        ...
        db_verified: bool, see main()'s own docstring for the full
            ...
            road_radio_db, dtm_radio_db, out_radio_db) if False; see
            that block's own comment for exactly why. When
            batch_mode=True, only road_radio_db/dtm_radio_db are built
        ...
        batch_mode: bool, default False -- NEW. When True, the Land
            ...
            DTM Source are the only sections still shown, since those
            are this tool's only other settings; "Run Processing" is
        ...
        initial_config: dict | None, default None -- NEW. Pre-fills
            Road Network Source and DTM Source (this tool's only
    ...
    apply_icon(win, "terrain.ico")
    win.title("Parcel Terrain Level Tool" + (" — Batch Mode" if batch_mode else ""))
    win.resizable(False, False)
    win.update_idletasks()
    win.deiconify()
    win.lift()
    ...
    # NEW -- batch mode: pre-fill Road Network Source and DTM Source
    ...
    if batch_mode and initial_config:
        ...
        if initial_config.get("dtm_source_type") == "db":
            dtm_source_type.set("db")
            dtm_db_table.set(initial_config.get("dtm_db_table") or "")
        else:
            dtm_source_type.set("local")
            dtm_local_path.set(initial_config.get("dtm_local_path") or "")

    # NEW -- placeholder assigned BEFORE Section 2 (Road Network) is
    # built below. Section 2's and Section 3's own handlers
    # (browse_road_file(), _on_road_db_selected(), _toggle_road(),
    # browse_dtm_file(), _on_dtm_db_selected(), _toggle_dtm()) call
    # _update_run_button_state() unconditionally -- the SAME code
    # whether batch_mode is True or False, since Road Network Source
    # and DTM Source are the two sections this tool always builds. In
    # batch mode there is no Run button to update, so this stays a
    # no-op; the REAL implementation (further below) is only defined --
    # shadowing this one -- inside the `if not batch_mode:` branch.
    # Safe: Python resolves this name at CALL time (when the user
    # actually interacts with a widget, after the whole window is
    # built), not at each function's own definition time.
    ...
    # db_verified block near the end of this function can disable it
    # when the session has no verified database connection.
    dtm_radio_db = tk.Radiobutton(
        dtm_radio_row, text="Database Table",
        variable=dtm_source_type, value="db",
        command=lambda: _toggle_dtm())
    dtm_radio_db.pack(side="left", padx=(12, 0))
    ...
    # DTM Source are the two sections this tool always builds, so their
    # own initial label sync and their own Database radio gating both
    # happen here, shared by both branches below, rather than
    # duplicated in each.
    _toggle_road()
    _toggle_dtm()
    if not db_verified:
        for _db_radio in (road_radio_db, dtm_radio_db):
    ...
    if not batch_mode:
        ...
        # parcel_radio_db, road_radio_db, dtm_radio_db, out_radio_db --
        ...
        # NEW -- batch mode: Road Network Source and DTM Source (built
        ...
        def _gather_batch_config():
            ...
            if dtm_source_type.get() == "local":
                config["dtm_source_type"] = "local"
                config["dtm_local_path"] = dtm_local_path.get()
            else:
                config["dtm_source_type"] = "db"
                config["dtm_db_table"] = dtm_db_table.get()
            return config
        ...
        try:
            ...
            while True:
                ...
                if kind == "phase":
                    ...
                    progress.close()
                    show_tool_dialog(app_root, "Success", rest[0], body_icon="success")
                    return
                elif kind == "error":
                    progress.close()
                    show_tool_dialog(app_root, "Error", rest[0], body_icon="error")
                    return
                elif kind == "cancelled":
                    progress.close()
